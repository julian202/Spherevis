...
selectWindow("Log");
print("////////////////////////");
visibilityString=d2s(visibility,0);
//showMessage(visibilityString);
...
function myfunc() { //the top max is maxN=0, the second max is maxN=1 and so on..   
	...
	while (i>=0) {
	    ...
	    if ((M+1)<ypoints.length){
			i=ypoints[M]-ypoints[M+1];
	   		M++;
		}
		else{
	    	i=-1;
	    	print("Didn't reach next min");
	    }
	   }
	...
	while (i>=0) {
		if ((M+1)<ypoints.length){
	    	...
	    	i=-1;
	    	print("Didn't reach second maximum");
	    }
